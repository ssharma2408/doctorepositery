...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.content-tags.update", [$contentTag->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="clinic_id">{{ trans('cruds.contentTag.fields.clinic') }}</label>
                <select class="form-control select2 {{ $errors->has('clinic') ? 'is-invalid' : '' }}" name="clinic_id" id="clinic_id" required>
                    @foreach($clinics as $id => $entry)
                        <option value="{{ $id }}" {{ (old('clinic_id') ? old('clinic_id') : $contentTag->clinic->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.contentTag.fields.clinic_helper') }}</span>
            </div>
            <div class="form-group">
