    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.closed-timings.update", [$closedTiming->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ClosedTiming::FULL_PARTIAL_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('full_partial') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="full_partial_{{ $key }}" name="full_partial" value="{{ $key }}" {{ old('full_partial', $closedTiming->full_partial) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('from') ? 'is-invalid' : '' }}" type="text" name="from" id="from" value="{{ old('from', $closedTiming->from) }}">
                @if($errors->has('from'))
                    <div class="invalid-feedback">
                        {{ $errors->first('from') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('to') ? 'is-invalid' : '' }}" type="text" name="to" id="to" value="{{ old('to', $closedTiming->to) }}">
                @if($errors->has('to'))
                    <div class="invalid-feedback">
                        {{ $errors->first('to') }}
                    </div>
